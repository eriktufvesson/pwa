...
const urlsToCache = [
    ...
    '/assets/imgs/cat-default.jpg',
  ];

self.addEventListener('install', (event) => {
  console.log('service worker about to be installed', event);
  
  event.waitUntil(
    caches.open(CURRENT_CACHES.assets)
      .then((cache) => {
        console.log('Opened the assets cache');
        // sendMessage({body: 'App Shell cached'});
...
//   //   })
//   // );
//   sendMessage({ title, body: 'New message' });
// });
...
function sendMessage(message) {
  self.clients.matchAll().then(function(clients) {
    clients.forEach(function(client) {
      console.log('sendMessage to client', client, message);
